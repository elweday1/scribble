...
const Guess = (props: {
  ...
  currentWord: string;
  currentRound: number;
  guessers: [string, number | undefined][];
  me: string;
  ...
  const isGuessAfter =
    guesserGuessTimestamp && timeStamp < guesserGuessTimestamp;
  ...
  const show =
    ...
    isMe ||
    me === currentDrawer ||
    ((!isGuessed || !isGuessAfter) && !meGuessed);
  ...
  return (
    show && (
      <li className="flex flex-col">
        <span
          className={cn(
            "origin-bottom-left scale-75 text-xs font-bold text-slate-300",
            {
              "text-slate-500": !name,
            },
          )}
...
export const Chat = () => {
  ...
  const currentRound =
    state.context.config.roundTime - state.context.roundsLeft;
  const currentDrawer = state.context.currentDrawer.toLocaleLowerCase();
  const guessers = Object.entries(state.context.players)
    .filter(([_, player]) => player.guessed === true)
    .map(([key, { timeStamp }]) => [key, timeStamp]) as [
    string,
    number | undefined,
  ][];
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col justify-between gap-3 rounded-xl bg-black/10 p-3">
      <ul className="flex min-h-0 w-full flex-1 flex-col-reverse gap-2 overflow-y-auto">
