...
export const local = storedAtom<Local>(
  ...
  { prefix: "_wordoodle_", storedKeys: ["avatar", "name"] }, // Removed "id" from storedKeys to ensure unique players per tab for testing
);
...
export type ExtractPaload<T extends EventName> = Omit<
  Extract<Event, { type: T }>,
  "type"
>;

export type Payload<T extends EventName> =
  ExtractPaload<T> extends Empty ? undefined : ExtractPaload<T>;

export type Events = Partial<{
  [K in EventName]: (cb: { payload: Payload<K> }) => void;
}>;
